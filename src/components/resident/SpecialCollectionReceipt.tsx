...
interface SpecialCollectionReceiptProps {
...
export function SpecialCollectionReceipt({
  contrib,
  request,
  house,
  onClose,
  ...
  const verifiedAtFormatted = contrib.verified_at
    ? formatDateTime(contrib.verified_at)
    : formatDateTime(contrib.submitted_at);
  const formattedAmount = formatCurrency(contrib.amount);
  const authId = contrib.id ? `${contrib.id.slice(0, 8)}...` : 'c0c9e604...';

  const handlePrint = () => {
    // Isolated iframe printing to guarantee 100% full-page rendering without blank pages in Microsoft Print to PDF
    let printFrame = document.getElementById('special-receipt-print-frame') as HTMLIFrameElement | null;
    if (printFrame) {
      printFrame.remove();
    }

    printFrame = document.createElement('iframe');
    printFrame.id = 'special-receipt-print-frame';
    ...
    const printHtml = `
      ...
      <html lang="en">
        <head>
          <meta charset="utf-8" />
          <title>${receiptNo} - Kunjikkulam Juma Masjid Special Contribution Receipt</title>
          <style>
            @page {
              size: A4 portrait;
